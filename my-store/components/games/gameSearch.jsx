 ...
 *  3) Filtrar la lista completa de juegos (`games`) cada vez que cambian los filtros.
 *  4) Renderizar:
 ...
 *     - Un grid responsivo de tarjetas (CardGame) con los juegos filtrados.
 *
 ...
 *     - En HorizontalFilter: escribe un texto → `handle(e)` → `onSearch(term)` → invoca `setSearchTerm(term)` en GameSearch.
 *     - En VerticalFilter: ajusta un control (slider, checkbox, fecha…) → llama al callback correspondiente (p.ej. `onPriceRangeChange([min,max])`) → actualiza el estado local en GameSearch.
 *  3) Cada vez que cualquiera de estos estados cambia, `useMemo` (ver más abajo) detecta la dependencia y:
 *     - Recalcula `filteredGames = games.filter(...)` usando todos los filtros actuales.
 *  4) React vuelve a renderizar:
 *     - HorizontalFilter recibe `productsShows={filteredGames.length}` y muestra el nuevo contador.
 *     - El grid de `CardGame` se mapea sobre `filteredGames` mostrando solo los juegos que cumplen todos los criterios.
 */
 ...
 *
 * - En nuestro caso:
 *   • Recalcula la lista filtrada `filteredGames` sólo si cambian:
...
export default function GameSearch() {
  ...
  const filteredGames = useMemo(() => {
    return games.filter(game => {
      ...
      if (!game.name.toLowerCase().includes(searchTerm.toLowerCase())) return false
      ...
      if (platforms.length && !platforms.every(p => game.platforms.includes(p))) return false
      ...
      const pubIso = game.release_date.split('/').reverse().join('-')
      ...
      if (discountOnly && game.discount === 0) return false
    ...
    <section className="px-4 pb-12">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1">
          {/* Barra de búsqueda */}
          <HorizontalFilter
            productsShows={filteredGames.length}
