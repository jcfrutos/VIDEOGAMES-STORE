 ...
 * Recibe los estados y setters del componente padre (GameSearch),
 * y al modificarlos actualiza la lista filtrada en GameSearch:
...
export default function VerticalFilter({
  ...
  const allGenres = ['Acción','Aventura','Rompecabezas','RPG','Simulación','Estrategia']
  const allPlatforms = ['PC','PlayStation','Xbox','Nintendo']
    ...
    ">
      ...
      <CardBody className="space-y-6 px-4">
          ...
          ">
            {allGenres.map(genre => (
              <Checkbox
                ...
                aria-label={`Filtrar por género ${genre}`}
              >
                {genre}
              </Checkbox>
            ))}
        ...
        <div>
          ...
          <RadioGroup
            value={selectedPlatforms.join(',')}  /* un solo valor: ej "PC" o "" */
            onValueChange={val => {
              // convertimos string a array: ["Xbox"] o []
          ...
          >
            {allPlatforms.map(p => (
              <Radio key={p} value={p} aria-label={`Filtrar plataforma ${p}`}>
                {p}
              </Radio>
            ))}
          </RadioGroup>
        ...
        <div>
          <Checkbox
            ...
            aria-label="Mostrar solo juegos con descuento"
          >
            Solo con descuento
          </Checkbox>
        </div>
